...
echo "Optional inference/API tunnel: http://127.0.0.1:$LOCAL_API_PORT -> $REMOTE_API_HEALTH_HOST:$REMOTE_API_PORT"
...
ssh -L "$LOCAL_API_PORT:$REMOTE_API_HEALTH_HOST:$REMOTE_API_PORT" \
  "${SSH_ARGS[@]}" \
  "$REMOTE_HOST"
